...
// #include "..\..\src\menu\resources\pinto.hpp"  // File not found - commented out
//-------------------------------------------------------------------------
// Warnings
//-------------------------------------------------------------------------
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    value->second = ImLerp( value->second, ( hovered && held ) ? 1.f : hovered ? 0.5f : 0.f, 0.045f );

    RenderFrame( bb.Min, bb.Max, to_vec4( 16, 16, 16, style.Alpha ), 0, 3 );
    RenderFrame( bb.Min, bb.Max, to_vec4( 22, 22, 22, style.Alpha * value->second ), 0, 3 );
    window->DrawList->AddRect( bb.Min, bb.Max, ImColor( 1.f, 1.f, 1.f, 0.06f * style.Alpha ), 3 );
    window->DrawList->AddRect( bb.Min, bb.Max, custom.get_accent_color( style.Alpha * value->second ), 3 );
...
bool ImGui::Checkbox( const char* label, bool* v ) {
    ...
    RenderFrame( frame_bb.Min, frame_bb.Max, ImColor( 24,24,24), 0, ROUNDING );
    window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, ImColor( 1.f, 1.f, 1.f, 0.06f * style.Alpha ), ROUNDING );
    RenderFrame( frame_bb.Min, frame_bb.Max, custom.get_accent_color( value->second * style.Alpha ), 0, ROUNDING );

    RenderCheckMark( window->DrawList, frame_bb.GetCenter( ) - ImVec2( ( ( square_sz / 2 + 1 ) * value->second ) / 2, ( ( square_sz / 2 + 1 ) * value->second ) / 2 ), ImColor(24,24,24), ( square_sz / 2 + 1 ) * value->second );

    ImVec2 label_pos = ImVec2( frame_bb.Max.x + style.ItemInnerSpacing.x + 2, frame_bb.GetCenter( ).y - label_size.y / 2 );
    if ( label_size.x > 0.f )
        window->DrawList->AddText( label_pos, GetColorU32( ImGuiCol_Text, value->second > 0.5f ? value->second : 0.5f ), label );
...
bool ImGui::Hotkey( const char* label, int* k ) {
    ...
    const ImVec2 size = ImVec2( CalcTextSize( KeyNames[*k] ).x + 15, 25 );
    const float size_ = GetWindowWidth( ) - 24;

    auto label_size = CalcTextSize( label, 0, 1 );

    ImRect frame_bb( window->DC.CursorPos + ImVec2( 0, label_size.y + 4 ), window->DC.CursorPos + ImVec2( size_, ( label_size.y + 28 ) ) );
    ImRect total_bb( window->DC.CursorPos, frame_bb.Max );
    ...
    RenderFrame( frame_bb.Min, frame_bb.Max, to_vec4( 13, 13, 13, style.Alpha ), 0, 3 );
    window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, ImColor( 1.f, 1.f, 1.f, 0.06f * style.Alpha ), 3 );
    ...
    ImGui::SetCursorPos(ImVec2( GetCursorPos() - ImVec2(-5, 31 ) ) );
    ImGui::Text(buf_display.c_str());
    
    //RenderTextClipped( frame_bb.Min, frame_bb.Max, buf_display.c_str( ), NULL, NULL, ImVec2( 0.5f, 0.5f ) );

    if ( label_size.x > 0 )
        RenderText( { total_bb.Min.x, frame_bb.GetCenter( ).y - label_size.y / 2 - 25 }, label );
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    ImRect frame_bb( window->DC.CursorPos + ImVec2( 0, label_size.y + 4 ), window->DC.CursorPos + ImVec2( size, ( label_size.y + 4 ) + 24 ) );
    ImRect total_bb( window->DC.CursorPos, frame_bb.Max );
    ...
    static std::unordered_map< ImGuiID, combo_ > values;
    auto value = values.find( id );
    if ( value == values.end( ) ) {
    ...
    value->second.popup = ImLerp( value->second.popup, ( popup_open ? 1.f : 0.f ), 0.038f );
    value->second.anim = ImLerp( value->second.anim, ( IsItemHovered( ) ? 1.f : 0.f ), 0.038f );

    window->DrawList->AddRectFilled( frame_bb.Min, frame_bb.Max, to_vec4( 13, 13, 13, style.Alpha ), 3 );
    window->DrawList->AddRectFilled( frame_bb.Min, frame_bb.Max, to_vec4( 19, 19, 19, style.Alpha * value->second.anim ), 3 );
    window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, ImColor( 1.f, 1.f, 1.f, 0.06f * style.Alpha ), 3 );
    window->DrawList->AddCircleFilled( ImVec2( frame_bb.Max.x - 10, frame_bb.GetCenter( ).y ), 3 + value->second.popup, custom.get_accent_color( style.Alpha ) );

    window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, ImColor( 1.f, 1.f, 1.f, 0.06f * style.Alpha ), 3 );

    RenderText( total_bb.Min, label );

    RenderTextClipped( ImVec2( frame_bb.Min.x + 8, frame_bb.GetCenter( ).y - label_size.y / 2 ), ImVec2( frame_bb.Max.x - 20, frame_bb.GetCenter( ).y + label_size.y / 2 ), preview_value, NULL, NULL, ImVec2( 0.f, 0.f ) );
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(w, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items) * value->second.popup));
    }
    ...
    PushStyleColor( ImGuiCol_PopupBg, ImVec4( ImColor( 13, 13, 13 ) ) );
    PushStyleColor( ImGuiCol_Border, ImVec4(1.f, 1.f, 1.f, 1.f) );
...
bool ImGui::SliderScalar( const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags ) {
    ...
    const ImVec2 label_size = CalcTextSize( label, 0, 1 );
    ImRect frame_bb( window->DC.CursorPos + ImVec2( 0, label_size.y + 6 ), window->DC.CursorPos + ImVec2( w, label_size.y + 16 ) );
    const ImRect total_bb( window->DC.CursorPos, frame_bb.Max );

    frame_bb.Min.y -=2; frame_bb.Max.y +=2;
    ...
    static std::unordered_map< ImGuiID, slider_ > values;
    auto value = values.find( id );
    ...
    value->second.grab = ImLerp( value->second.grab, percent * frame_bb.GetWidth( ), 0.125f );
    value->second.anim = ImLerp( value->second.anim, IsItemActive( ) ? 0.5f : IsItemHovered( ) ? 0.25f : 0.f, 0.045f );

    window->DrawList->AddRectFilled( frame_bb.Min, frame_bb.Max, to_vec4( 13, 13, 13, style.Alpha ), 5 );

    if ( hovered )
        window->DrawList->AddRectFilled( frame_bb.Min, ImVec2( GetMousePos( ).x, frame_bb.Max.y ), custom.get_accent_color( style.Alpha * 0.1f ), 5 );

    window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, ImColor( 1.f, 1.f, 1.f, 0.06f * style.Alpha ), 5 );
    window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, custom.get_accent_color( style.Alpha * value->second.anim ), 5 );
    window->DrawList->AddRectFilled( frame_bb.Min, ImVec2( frame_bb.Min.x + value->second.grab + 5, frame_bb.Max.y ), custom.get_accent_color( style.Alpha ), 5 );

    PushStyleColor( ImGuiCol_Text, GetColorU32( ImGuiCol_Text, 0.5f ) );
    RenderText( total_bb.Min, label );
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    return InputTextEx(label, NULL, buf, (int)buf_size, ImVec2(GetWindowWidth() - 24, 25), flags, callback, user_data);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y + 6), window->DC.CursorPos + frame_size + ImVec2(0, label_size.y + 6));
    const ImRect total_bb(window->DC.CursorPos, frame_bb.Max);
    ...
    value->second = ImLerp( value->second, IsItemActive( ) ? 1.f : IsItemHovered( ) ? 0.5f : 0.f, 0.05f );

    // Render frame
    if (!is_multiline)
    {
        RenderNavHighlight(frame_bb, id);
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, to_vec4(13, 13, 13, GetStyle().Alpha), 3);
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, to_vec4(19, 19, 19, GetStyle().Alpha * value->second), 3);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(1.f, 1.f, 1.f, 0.06f * GetStyle().Alpha), 3);
    }

    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : ImVec2( frame_bb.Min.x + style.FramePadding.x, frame_bb.GetCenter( ).y - label_size.y / 2 );
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags, ImVec2(16, 16)))
        {
        ...
        PushStyleColor( ImGuiCol_PopupBg, ImVec4( ImColor( 13, 13, 13 ) ) );
        PushStyleColor( ImGuiCol_Border, ImVec4( 1.f, 1.f, 1.f, 0.06f ) );
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float bars_width = 6; // Arbitrary smallish width of Hue/Alpha picking bars
    ...
    if ( Button( "Copy", ImVec2( GetWindowWidth( ) / 2 - GetStyle( ).ItemSpacing.x / 2 - GetStyle( ).WindowPadding.x, 0 ) ) )
        memcpy( custom.color_buf, col, components * sizeof( float ) );

    SameLine( );

    if ( Button( "Paste", ImVec2( GetWindowWidth( ) / 2 - GetStyle( ).ItemSpacing.x / 2 - GetStyle( ).WindowPadding.x, 0 ) ) ) {

        memcpy( col, custom.color_buf, components * sizeof( float ) );
        value_changed = true;
    }
    ...
    {
        // Render SV Square
        draw_list->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), ImColor( 13, 13, 13 ), col_white, hue_color32, hue_color32, col_white, 4);
        draw_list->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), ImColor( 13, 13, 13 ), 0, 0, col_black, col_black, 4);
    ...
    if (alpha_bar)
    {
        ...
        draw_list->AddRectFilledMultiColorRounded(bar1_bb.Min, bar1_bb.Max, ImColor( 13, 13, 13 ), ImColor( col[0], col[1], col[2], style.Alpha ), ImColor( col[0], col[1], col[2], 0.f * style.Alpha ), ImColor( col[0], col[1], col[2], 0.f * style.Alpha ), ImColor( col[0], col[1], col[2], style.Alpha ), 3 );
        float bar1_line_x = IM_ROUND(picker_pos.x + (1.0f - alpha) * sv_picker_size);
        custom.render_rect_for_horizontal_bar( ImVec2( bar1_line_x + 1, bar1_bb.GetCenter( ).y ), ImColor( col[0], col[1], col[2] ), style.Alpha );
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    value->second = ImLerp( value->second, ( selected ? 1.f : hovered ? 0.5f : 0.f ), 0.04f );

    // Render
    RenderFrame( bb.Min, bb.Max, ImColor( 1.f, 1.f, 1.f, ( 0.02f * value->second ) * style.Alpha ), false, 0.0f );
    ...
    // GetColorU32( ImGuiCol_Text, selected ? 1.f : hovered ? 0.5f : 0.25f )

    if ( selected )
        PushStyleColor( ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 1.f) );
    else
        PushStyleColor( ImGuiCol_Text, GetColorU32( ImGuiCol_Text, selected ? 1.f : hovered ? 0.5f : 0.25f ) );

    RenderTextClipped( text_min + ImVec2( 8, 0 ), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb );
    PopStyleColor( );
