#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: BP_CC_UniqueSeal

#include "Basic.hpp"

#include "CommonModule_structs.hpp"
#include "Engine_structs.hpp"
#include "InGameModule_classes.hpp"


SDK_NAMESPACE_START

// BlueprintGeneratedClass BP_CC_UniqueSeal.BP_CC_UniqueSeal_C
// 0x0008 (0x0158 - 0x0150)
class UBP_CC_UniqueSeal_C final : public UCharacterConditionBase
{
public:
	struct FPointerToUberGraphFrame               UberGraphFrame;                                    // 0x0150(0x0008)(ZeroConstructor, Transient, DuplicateTransient)

public:
	void ExecuteUbergraph_BP_CC_UniqueSeal(int32 EntryPoint);
	void BP_OnHiddenCondition();
	void BP_OnVisibleCondition();
	void BP_OnEndCondition();
	void BP_OnBeginCondition();
	void BP_OnUpdateIntervalValue(float value);

public:
	static class UClass* StaticClass()
	{
		BP_STATIC_CLASS_IMPL("BP_CC_UniqueSeal_C")
	}
	static const class FName& StaticName()
	{
		STATIC_NAME_IMPL(L"BP_CC_UniqueSeal_C")
	}
	static class UBP_CC_UniqueSeal_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UBP_CC_UniqueSeal_C>();
	}
};

SDK_NAMESPACE_END
